...
for (i in 1:5000){
  ...
  dat$M2_MNAR_prob<- exp(M2_logit)/(1+ exp(M2_logit))
  dat$M2_MNAR<- as.numeric(dat$M2_MNAR_prob>= runif(n)) # missing indicator under MAR
  
  
  dat<- dat%>% mutate(
    X2_observed_MNAR= case_when(
      M2_MNAR== 1~ NA_real_,
      M2_MNAR== 0~ X2
    )
  )
  ...
  # Potential outcome
  # Y(1)
  dat$Y1<- 1+ dat$X1+ dat$X2+ dat$M2_MNAR+ (2)*1+ rnorm(n, mean= 0, sd= 1)
  # Y(0)
  dat$Y0<- 1+ dat$X1+ dat$X2+ dat$M2_MNAR+ (2)*0+ rnorm(n, mean= 0, sd= 1)
  ...
  dat<- dat%>% mutate(
    X2_imputeMean_MNAR= case_when(
      is.na(X2_observed_MNAR)~ mean(dat$X2_observed_MNAR, na.rm= TRUE),
      TRUE~ X2_observed_MNAR
    )
  )
  
  # for the missing indicator method ----
  dat<- dat%>% mutate(
    X2_impute0_MNAR= case_when(
      is.na(X2_observed_MNAR)~ 0,
      TRUE~ X2_observed_MNAR
    )
  )
  
  # for multiple imputation with baseline variables ----
  imputationVar<- c("X1", "X2_observed_MNAR") 
  ...
  dat_multiple_MNAR_designStage<- complete(imp.dat, "long")%>% 
    group_by(.imp)%>% 
  ...
  imputationVar<- c("X1", "X2_observed_MNAR", "Y_experiment", "T") 
  ...
  dat_multiple_MNAR_overall<- complete(imp.dat, "long")%>% 
    group_by(.imp)%>% 
    mutate(cbind(dat_rest))%>% 
    ungroup()
  ...
  imputationVar<- c("X1", "X2_observed_MNAR", "Y_experiment") 
  ...
  dat_multiple_MNAR_bygroup<- rbind(aa, bb)
  ...
  dat<- dat%>% mutate(
    X1_predi= X1- mean(X1),
    X2_predi= X2_observed_MNAR- mean(X2_observed_MNAR, na.rm= TRUE) 
  )
  ...
  dat<- dat%>% mutate(
    X1_predi= X1- mean(X1), 
    X2_predi= X2_imputeMean_MNAR- mean(X2_imputeMean_MNAR, na.rm= TRUE), 
  )
  ...
  dat<- dat%>% mutate(
    X1_predi= X1- mean(X1), 
    X2_predi= X2_impute0_MNAR- mean(X2_impute0_MNAR, na.rm= TRUE), 
    M2_predi= M2_MNAR- mean(M2_MNAR)
  )
  ...
  # model 1
  lm<- dat_multiple_MNAR_designStage%>%
    group_by(.imp)%>%
    mutate(X1_predi= X1- mean(X1),
           X2_predi= X2_observed_MNAR- mean(X2_observed_MNAR))%>%
